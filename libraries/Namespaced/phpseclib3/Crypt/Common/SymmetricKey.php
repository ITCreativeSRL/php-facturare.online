...
/**
 * Base Class for all \FacturareOnline\phpseclib3\Crypt\* cipher classes
 *
 * PHP version 5
 *
 ...
 *
 *  - The new \FacturareOnline\phpseclib3\Crypt\* cipher class should extend \FacturareOnline\phpseclib3\Crypt\Common\SymmetricKey
 *
 ...
 *
 *  - Look at the source code of the current ciphers how they extend \FacturareOnline\phpseclib3\Crypt\Common\SymmetricKey
...
namespace FacturareOnline\phpseclib3\Crypt\Common;
...
use FacturareOnline\phpseclib3\Exception\BadDecryptionException;
use FacturareOnline\phpseclib3\Exception\BadModeException;
use FacturareOnline\phpseclib3\Exception\InconsistentSetupException;
...
use FacturareOnline\phpseclib3\Math\BinaryField;
use FacturareOnline\phpseclib3\Math\PrimeField;
...
abstract class SymmetricKey
{
     ...
     * @see \FacturareOnline\phpseclib3\Crypt\Common\SymmetricKey::encrypt()
     * @see \FacturareOnline\phpseclib3\Crypt\Common\SymmetricKey::decrypt()
     */
    const MODE_CTR = -1;
    /**
     ...
     * Mode Map
     *
     * @see \FacturareOnline\phpseclib3\Crypt\Common\SymmetricKey::__construct()
     */
    const MODE_MAP = [
        'ctr'    => self::MODE_CTR,
        'ecb'    => self::MODE_ECB,
     ...
     * Engine Reverse Map
     *
     * @see \FacturareOnline\phpseclib3\Crypt\Common\SymmetricKey::getEngine()
     */
    const ENGINE_MAP = [
     ...
     *
     * @see \FacturareOnline\phpseclib3\Crypt\Twofish::setKey()
     * @see \FacturareOnline\phpseclib3\Crypt\Twofish::setIV()
     * @var bool
     */
     ...
     * @see self::setPassword()
     * @param int $n
     * @param \FacturareOnline\phpseclib3\Crypt\Hash $hashObj
     * @param string $i
     * @param string $d
     * @param int $count
     * @return string $a
     ...
     *
     * Put another way, when the continuous buffer is enabled, the state of the \FacturareOnline\phpseclib3\Crypt\*() object changes after each
     ...
     * Encrypts a block
     *
     * Note: Must be extended by the child \FacturareOnline\phpseclib3\Crypt\* class
     *
     * @param string $in
     * @return string
     */
     ...
     *
     * Note: Must extend by the child \FacturareOnline\phpseclib3\Crypt\* class
     *
     * @see self::setup()
     */
     ...
     *       - In case of using inline crypting, _setupInlineCrypt() must extend by the child \FacturareOnline\phpseclib3\Crypt\* class.
     ...
     *    </code>
     *
     *    See also the \FacturareOnline\phpseclib3\Crypt\*::_setupInlineCrypt()'s for
                    ...
                    if (strlen($_buffer["ciphertext"])) {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= ' . $block_size . ') {
                            ...
                            if (strlen($_block) > strlen($_buffer["ciphertext"])) {
                                $in = $_xor;
                                ' . $encrypt_block . '
                                \FacturareOnline\phpseclib3\Common\Functions\Strings::increment_str($_xor);
                                $_buffer["ciphertext"].= $in;
                            }
                            $_key = \FacturareOnline\phpseclib3\Common\Functions\Strings::shift($_buffer["ciphertext"], ' . $block_size . ');
                ...
                $encrypt = $init_encrypt . '
                    ...
                    if (strlen($_buffer["xor"])) {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= ' . $block_size . ') {
                            ...
                            $_key = \FacturareOnline\phpseclib3\Common\Functions\Strings::shift($_buffer["xor"], ' . $block_size . ');
                            $_ciphertext.= $_block ^ $_key;
                        }
                    } else {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= ' . $block_size . ') {
