...
namespace FacturareOnline\phpseclib3\File;
...
use FacturareOnline\phpseclib3\Crypt\PublicKeyLoader;
use FacturareOnline\phpseclib3\Crypt\Random;
use FacturareOnline\phpseclib3\Crypt\RSA;
use FacturareOnline\phpseclib3\Crypt\RSA\Formats\Keys\PSS;
use FacturareOnline\phpseclib3\Exception\UnsupportedAlgorithmException;
use FacturareOnline\phpseclib3\File\ASN1\Element;
use FacturareOnline\phpseclib3\File\ASN1\Maps;
...
class X509
{
     ...
     *
     * @see \FacturareOnline\phpseclib3\File\X509::getDN()
     */
    const DN_ARRAY = 0;
    /**
     * Return string
     ...
     *
     * @see \FacturareOnline\phpseclib3\File\X509::saveX509()
     * @see \FacturareOnline\phpseclib3\File\X509::saveCSR()
     * @see \FacturareOnline\phpseclib3\File\X509::saveCRL()
     */
    const FORMAT_PEM = 0;
    /**
     * Save as DER
     ...
     *
     * There's no guarantee \FacturareOnline\phpseclib3\File\X509 is going to re-encode an X.509 cert in the same way it was originally
     ...
     * Default Constructor.
     *
     * @return \FacturareOnline\phpseclib3\File\X509
     */
    ...
    public function saveX509(array $cert, $format = self::FORMAT_PEM)
    {
        ...
        /* in the case of policyQualifiers/qualifier, the type has to be \FacturareOnline\phpseclib3\File\ASN1::TYPE_IA5_STRING.
           \FacturareOnline\phpseclib3\File\ASN1::TYPE_PRINTABLE_STRING will cause OpenSSL's X.509 parser to spit out random
           characters.
         */
                    ...
                    case 'id-ce-certificatePolicies':
                        for ($j = 0; $j < count($value); $j++) {
                            ...
                            for ($k = 0; $k < count($value[$j]['policyQualifiers']); $k++) {
                                ...
                                $subvalue = &$value[$j]['policyQualifiers'][$k]['qualifier'];
                                if ($map !== false) {
                                    // by default \FacturareOnline\phpseclib3\File\ASN1 will try to render qualifier as a \FacturareOnline\phpseclib3\File\ASN1::TYPE_IA5_STRING since it's
                                    // actual type is \FacturareOnline\phpseclib3\File\ASN1::TYPE_ANY
    ...
    {
        if (!is_string($extnId)) { // eg. if it's a \FacturareOnline\phpseclib3\File\ASN1\Element object
            return true;
        }
     ...
     * @throws \FacturareOnline\phpseclib3\Exception\UnsupportedAlgorithmException if the algorithm is unsupported
     * @return bool
     */
     ...
     * Set public key
     *
     * Key needs to be a \FacturareOnline\phpseclib3\Crypt\RSA object
     *
     * @param PublicKey $key
     * @return void
     */
     ...
     * Gets the public key
     *
     * Returns a \FacturareOnline\phpseclib3\Crypt\RSA object or a false.
     *
     * @return mixed
     */
    ...
    public function sign(X509 $issuer, X509 $subject)
    {
        ...
        if (isset($subject->domains) && count($subject->domains)) {
            $altName = array_map(['\FacturareOnline\phpseclib3\File\X509', 'dnsName'], $subject->domains);
        }
        ...
        // save $tbsCertificate in case there are any \FacturareOnline\phpseclib3\File\ASN1\Element objects in it
    ...
    public function signCSR()
    {
        ...
        // resync $this->signatureSubject
        // save $certificationRequestInfo in case there are any \FacturareOnline\phpseclib3\File\ASN1\Element objects in it
    ...
    public function signSPKAC()
    {
        ...
        // resync $this->signatureSubject
        // save $publicKeyAndChallenge in case there are any \FacturareOnline\phpseclib3\File\ASN1\Element objects in it
    ...
    public function signCRL(X509 $issuer, X509 $crl)
    {
        ...
        // resync $this->signatureSubject
        // save $tbsCertList in case there are any \FacturareOnline\phpseclib3\File\ASN1\Element objects in it
        $tbsCertList = $this->currentCert['tbsCertList'];
     ...
     * - Key object
     * - \FacturareOnline\phpseclib3\File\X509 object with public or private key defined
     * - Certificate or CSR array
     * - \FacturareOnline\phpseclib3\File\ASN1\Element object
     * - PEM or DER string
     *
    ...
    public function computeKeyIdentifier($key = null, $method = 1)
    {
        ...
        switch (true) {
            ...
            case $key instanceof X509:
                ...
                return false;
            default: // Should be a key object (i.e.: \FacturareOnline\phpseclib3\Crypt\RSA).
